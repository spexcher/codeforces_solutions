...
vll par(1010);
ll n;
pll center_diameter(ll node)
{
    ll cnt = 0;
    auto dfs = [&](ll node, ll parent, ll depth, ll dis, auto &&self) -> void
    {
        dep[node] = depth;
        vis[node] = 1;
        par[node] = parent;
        ...
        {
            if (nbr != parent)
                self(nbr, node, depth + 1, dis, self);
        }
    };
    dep.assign(sz(dep), 0);
    dfs(node, 0, 0, -1, dfs);
    ll maxchild = node;
    ff(i, 1, n) if (dep[i] > dep[maxchild])
        maxchild = i;
    dep.assign(sz(dep), 0);
    dfs(maxchild, 0, 0, -1, dfs);
    ll maxd = 0;
    ll num = node;
    ff(i, 1, n) if (dep[i] > maxd)
        num = i,
        maxd = max(maxd, dep[i]);
    ll c1;
    ff(i, 1, maxd / 2)
...
void solve()
{
    ...
    vpll dc;
    ff(i, 1, n) if (!vis[i])
        dc.pb({center_diameter(i).S, center_diameter(i).F});
    sort(all(dc));
    ll center = dc.back().S, diam = dc.back().F;
    dc.ppb();
    vpll pairs;
    for (auto [a, b] : dc)
        pairs.pb({center, b}), t[center].pb(b), t[b].pb(center);
    print(center_diameter(1).S);
    print(pairs);
}
/* ████████╗██╗  ██╗ █████╗ ███╗   ██╗██╗  ██╗    ██╗   ██╗ ██████╗ ██╗   ██╗     */
/* ╚══██╔══╝██║  ██║██╔══██╗████╗  ██║██║ ██╔╝    ╚██╗ ██╔╝██╔═══██╗██║   ██║     */
