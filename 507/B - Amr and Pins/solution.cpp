...
// From Contest: Codeforces - Codeforces Round 287 (Div. 2)
...
void solve() {
 ...
 cin>>r>>x1>>y1>>x2>>y2;
 
 double distance =sqrt(abs(x2-x1)*abs(x2-x1) +abs(y2-y1)*abs(y2-y1));
 double u=ceil((distance-2*r)/(2*r)+1);
 if(distance<=(2*r) and distance!=0){print(1);return;}
 if(distance<=0){print(0);return;}
  print((int)u);
