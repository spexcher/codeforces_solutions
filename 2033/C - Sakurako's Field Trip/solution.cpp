...
void solve()
{
    ...
    ff(i, 1, ((n + 1) / 2) - 1)
    {
        dp[i][0] = min(dp[i - 1][0] + (v[i - 1] == v[i]) + (v[n - i] == v[n - i - 1]), dp[i - 1][1] + (v[n - i - 1] == v[i]) + (v[n - i] == v[i - 1]));
        dp[i][1] = min(dp[i - 1][0] + (v[n - i - 1] == v[i]) + (v[n - i] == v[i - 1]), dp[i - 1][1] + (v[i - 1] == v[i]) + (v[n - i] == v[n - i - 1]));
    }
    pr(dp);
    ll res = min(dp[s - 1][0], dp[s - 1][1]);
    if (n % 2 == 0)
        res += (v[n / 2] == v[n / 2 - 1]);
    print(res);
}
