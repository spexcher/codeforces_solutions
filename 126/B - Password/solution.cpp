...
void solve()
{
    ...
    ff(i, 1, n - 1)
    {
        hash[i] = (hash[i - 1] % mod + ((s[i] - 'a' + 1) * pow) % mod) % mod;
        power[i] = pow;
        pow = (pow * mul) % mod;
    }
    // pr(hash);
    ...
    auto cmp = [&](int l1, int r1, int l2, int r2) -> bool
    {
        int res1 = hash[r1];
        if (l1 > 0)
            res1 = (res1 - hash[l1 - 1] + mod) % mod;
        int res2 = hash[r2];
        if (l2 > 0)
            res2 = (res2 - hash[l2 - 1] + mod) % mod;
        // pr(l1, r1, l2, r2);
        res1 = ((res1 % mod) * (power[l2] % mod)) % mod;
        // pr(res1);
        res2 = ((res2 % mod) * (power[l1] % mod)) % mod;
        // pr(res2);
        return (res1 == res2);
    };
    ll res = -1;
    ff(i, 0, n - 2)
    {
        bool test = cmp(0, i, n - i - 1, n - 1);
        // pr(i);
        // pr(test);
        if (test)
            ff(j, 1, n - 2)
            {
                bool check = cmp(0, i, j, min(i + j, n - 2));
                // pr(j);
                // pr(check);
                if (check)
                {
