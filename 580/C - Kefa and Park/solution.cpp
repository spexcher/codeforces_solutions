...
// From Contest: Codeforces - Codeforces Round 321 (Div. 2)
...
// Institution: Indian Institute of Technology Madras(BS UG Data Science 4 year)
...
int ans = 0;
int n, m;
void dfs(int node, int parent, int maxcat, int c_cat) {
  if (cat[node] == 0)
    c_cat = 0;
  else
    c_cat++;
  maxcat = max(maxcat, c_cat);
  ...
  for (auto it : g[node]) {
    ...
    if (it != parent) {
      nchild++;
      dfs(it, node, maxcat, c_cat);
    }
  }
  if (nchild == 0 and maxcat <= m) ans++;
}
 
void solve() {
  cin >> n >> m;
  Fo(i, 1, n + 1) cin >> cat[i];
  int u, v;
  Fo(i, 1, n) {
    cin >> u >> v;
    g[v].pb(u);
