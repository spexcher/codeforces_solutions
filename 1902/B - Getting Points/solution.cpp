...
// From Contest: Codeforces - Educational Codeforces Round 159 (Rated for Div. 2)
...
signed main()
{
 ...
 int t = 1;
 cin >> t;
 Fo(tc,1,t) {  //eprintf("--- Case #%lld start ---
...
void solve(int tc) {
 ...
 cin>>n>>p>>l>>t;
 auto poss = [&](int mid) {
  int total_quest=ceildiv(n,7);
  return ((mid*l+min(total_quest,2*mid)*t)>=p);
  };
  int lo = 0;
  int hi = n;
  int res = n;
