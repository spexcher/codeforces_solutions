...
void solve()
{
    ...
    auto build = [&](int index, int l, int r, auto &&self) -> void
    {
        ...
        gcd_segtree[index] = gcd_segtree[2 * index + 1] + gcd_segtree[2 * index + 2]; // combine logic
    };
...
 
        gcd_segtree[index] = gcd_segtree[2 * index + 1] + gcd_segtree[2 * index + 2]; // combine logic                                                                                      // apply logic
    };
    ...
    ff(i, 0, n - 1)
    {
        if ((i + 1) > v[i])
        {
            res += query(0, 0, n - 1, 1, v[i] - 1, query);
            update(0, 0, n - 1, (i + 1), 1, update);
        }
        // ll prev = query(0, 0, n - 1, v[i] - 1, v[i] - 1, query);
    }
    print(res);
}
 
