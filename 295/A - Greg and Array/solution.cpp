...
void solve()
{
    ...
    for (auto [l, r] : ops2)
        pre2[r + 1]--, pre2[l]++;
    ff(i,1,M)pre2[i]+=pre2[i-1];
    //print(pre2);
    vll mask(N+1);
    ff(i,0,M-1){
        int times=pre2[i];
        mask[get<0>(ops[i])]+=times*get<2>(ops[i]);
        mask[(get<1>(ops[i]))+1]-=times*get<2>(ops[i]);
        // print(get<0>(ops[i]));
        // print(get<1>(ops[i]));
        //print(get<2>(ops[i]));
    }
    ff(i,1,N)mask[i]+=mask[i-1];
    ff(i,0,N-1)mask[i]+=v[i];
    mask.ppb();
    print(mask);
}