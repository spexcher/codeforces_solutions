...
map<char, pll> movem = {{'U', {-1, 0}}, {'L', {0, -1}}, {'D', {1, 0}}, {'R', {0, 1}}};
void solve()
{
    int n, m;
    cin >> n >> m;
    ...
    vvb dp(n, vb(m, 1));
    cin >> grid;
    auto dfs = [&](int i, int j, auto &&self) -> bool
    {
        if (!isValid(i, j, n, m))
            return false;
        if (vis[i][j])
        ...
        if (grid[i][j] == '?')
        {
            bool found = 0;
            for (auto [a, b] : movem)
                if (self(i + b.F, j + b.S, self))
                    found = 1;
            dp[i][j] = found;
        }
        else
            dp[i][j] = self(i + movem[grid[i][j]].F, j + movem[grid[i][j]].S, self);
        return dp[i][j];
    };
    ff(i, 0, n - 1) ff(j, 0, m - 1) if (!vis[i][j]) dfs(i, j, dfs);
    ll res = 0;
    ff(i, 0, n - 1) ff(j, 0, m - 1) res += dp[i][j];
    print(res);
}