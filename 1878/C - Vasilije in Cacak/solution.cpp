...
//-----------------------------------------End Snippet----------------------------------------
bool canExpressAsSum(int n, int k, int x)
{
    // If k is greater than n or x is outside the possible range, it's not possible
    if (k > n || x < k || x > (n * (n + 1)) / 2)
    {
        return false;
    }
 
    // Create a vector to store the selected elements
    std::vector<int> selected;
 
    // Start with the largest possible element and work backwards
    for (int i = n; i >= 1; i--)
    {
        if (x - i >= k - 1)
        {
        ...
        if (k == 0 && x == 0)
        {
            return true; // Found k distinct elements that sum to x
        }
    }
 
    return false; // If we reach here, it's not possible
}
void solve()
{
    int n, k, x;
    cin >> n >> k >> x;
    int maxthroughk = k * (k + 1) / 2;
    debug(maxthroughk);
    int full = n * (n + 1) / 2;
    int kmin = 2 * n - k + 1;
    debug(kmin);
    // bool ok = true;
    if ((maxthroughk > x || kmin * k / 2 < x))
    {
        pn;
    }
    else if (x <= full)
