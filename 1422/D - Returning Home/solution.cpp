// ------------------------------------------------------------------------------------
// Problem Name: D. Returning Home
// From Contest: Codeforces - Codeforces Round 675 (Div. 2)
...
const int N = 100001;
ll n, m, sx, sy, ex, ey, a, b;
vector<pair<int,ll>> g[N];         
vector<pair<ll,int>> X, Y;             
vector<ll> dist;
vector<bool> vis;
void addEdges(vector<pair<ll,int>> &coord) {          
    for(int i=1; i<sz(coord); i++) {
        int id1 = coord[i - 1].ss, id2 = coord[i].ss;
        ll w = coord[i].ff - coord[i - 1].ff;
        g[id1].pb({id2, w});
        g[id2].pb({id1, w});
    }
}
void solve() {
    ...
    for(int i=0; i<m; i++) {
        ll w = min(abs(sx - X[i].ff), abs(sy - Y[i].ff));
        g[0].pb({X[i].ss, w});                        
    }
    for(int i=0; i<m; i++) {
        ll w = abs(ex - X[i].ff) + abs(ey - Y[i].ff);            
        g[X[i].ss].pb({m + 1, w});                  
    }
    ...
    while(!pq.empty()) {
        ...
        if(vis[curr.ss]) continue;
        int id = curr.ss;
        ll w = curr.ff;
        vis[id] = true;
        dist[id] = w;
        for(auto edge: g[id])
            if(dist[edge.ff] > w + edge.ss)
                pq.push({w + edge.ss, edge.ff});
    }
    print(dist[m + 1]);
}