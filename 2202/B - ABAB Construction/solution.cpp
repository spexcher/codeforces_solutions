...
//  Created      : 10.04.2026 at 11:11:44
...
void solve()
{
    ...
    string t;
    cin >> t;
    auto rec = [&](auto &&self, int l, int r, int idx) -> bool
    {
        if (idx == n)
            rt;
        if (t[idx] != '?')
        {
            if (s[l] == t[idx])
                return self(self, l + 1, r, idx + 1);
            if (s[r] == t[idx])
                return self(self, l, r - 1, idx + 1);
            else
                rf;
        }
        else if (t[idx] == '?')
        {
            return self(self, l + 1, r, idx + 1) or self(self, l, r - 1, idx + 1);
        }
        else
            rf;
    };
    rec(rec, 0, n - 1, 0) ? py : pn;
}
// ======================================================
