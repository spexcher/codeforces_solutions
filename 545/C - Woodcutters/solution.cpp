...
void solve()
{
    ...
    function<int(int, int)> rec = [&](int idx, int prev) -> int
    {
        if (idx == n - 1)
            return 1ll;
        if (idx == 0)
            return 1ll + rec(idx + 1, 0);
        if (dp[idx][prev] != -1)
            return dp[idx][prev];
        int left = 1ll + rec(idx + 1, 0);
        int right = 1ll + rec(idx + 1, 1);
        int not_fallen = rec(idx + 1, 0);
        int res = 0ll;
        bool left_condition_falled = (ih[idx - 1].F + ih[idx - 1].S) < (ih[idx].F - ih[idx].S);
        bool left_condition = (ih[idx - 1].F) < (ih[idx].F - ih[idx].S);
        bool right_condition = (ih[idx + 1].F) > (ih[idx].F + ih[idx].S);
        // left
        if (prev and left_condition_falled)
            res = max(res, left);
        else if (!prev and left_condition)
            res = max(res, left);
        // right
        if (right_condition)
            res = max(res, right);
        res = max(res, not_fallen);
