...
//-----------------------------------------End Snippet----------------------------------------
 
int maxWater(vector<int> arr, int n, int proposition)
{
    int water = 0;
    for (int i = 0; i < n; i++)
    {
        if (arr[i] < proposition)
            water += (proposition - arr[i]);
    }
    return water;
}
void solve()
{
   ...
   // deque<int> v(x);
    ...
    {
        proposition = l + ((r - l) >> 2);
        // v.push_front(proposition);
        // v.push_back(proposition);
        if (maxWater(v, v.size(), proposition) <= y)
        {
            res = max(proposition,res);
            l = proposition + 1;
        }
        else
            r = proposition - 1;
