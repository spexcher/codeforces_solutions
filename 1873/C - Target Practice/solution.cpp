...
void solve()
{
    vector<vector<char>> v(11, vector<char>(11, 't'));
    int sum = 0;
    for (int i = 1; i < 11; i++)
    {
        for (int j = 1; j < 11; j++)
    ...
    for (int i = 1; i < 11; i++)
    {
        for (int j = 1; j < 11; j++)
        {
            if (v[i][j] != '.')
            {
                int distx = min(abs(i - 5), abs(i - 6));
                int disty = min(abs(j - 5), abs(j - 6));
                int distz = max(distx,disty);
                if(distz==0)sum+=5;
                else if(distz==1)sum+=4;
                else if (distz==2)sum+=3;
                else if(distz==3)sum+=2;
                else if(distz==4)sum+=1;
            }
        }
    }
    cout << sum << nl;
