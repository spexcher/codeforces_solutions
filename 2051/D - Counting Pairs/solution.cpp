...
void solve()
{
    ...
    sort(all(v));
    ll sum = sum(v);
    auto countpairslessorequaltarget = [&](ll target) -> ll
    {
        vll temp;
        ll cnt = 0;
        for(auto x : v)
        {
            cnt += upper_bound(all(temp), target - x) - begin(temp);
            temp.pb(x);
        }
        return cnt;
    };
    print(countpairslessorequaltarget(sum - x) - countpairslessorequaltarget(sum - y - 1));
}
 
/* ████████╗██╗  ██╗ █████╗ ███╗   ██╗██╗  ██╗    ██╗   ██╗ ██████╗ ██╗   ██╗     */
/* ╚══██╔══╝██║  ██║██╔══██╗████╗  ██║██║ ██╔╝    ╚██╗ ██╔╝██╔═══██╗██║   ██║     */
