...
void solve()
{
    ...
    cin >> a >> b;
    // pr(a,b);
    mll positive, negative;
    // haha sweepline
    ff(i, 0, n - 1) positive[1]++, positive[b[i] + 1]--, negative[a[i] + 1]++, negative[b[i] + 1]--;
    // pr(positive);
    // pr(negative);
    ll dummy = 0;
    for (auto &[a, b] : positive)
    {
        dummy += b;
        b = dummy;
    }
    dummy = 0;
    for (auto &[a, b] : negative)
    {
        dummy += b;
        b = dummy;
    }
    ...
    for (ll price : prices)
    {
        pll positive_reviews = *positive.lower_bound(price);
        pll negative_reviews = *negative.lower_bound(price);
        ll money = 0ll;
        if (price == positive_reviews.F)
            money = positive_reviews.S;
        else
        {
            money = (--positive.lower_bound(price))->S;
        }
        ll bad = 0ll;
        if (price == negative_reviews.F)
            bad = negative_reviews.S;
        else
            bad = (--negative.lower_bound(price))->S;
        //pr(price, money, bad);
        if (bad <= k)
        {
            dkk = max(dkk, money * price);
        }
    }
    print(dkk);
    //pr("--------------------------------");
