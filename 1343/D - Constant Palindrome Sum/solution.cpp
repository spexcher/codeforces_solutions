...
void solve()
{
    ...
    ff(i, 0, n / 2 - 1)
    {
        ll mi = min(v[i], v[n - i - 1]);
        ll ma = max(v[i], v[n - i - 1]);
        sums[mi + ma]++;
        maxvals.pb(ma);
        minvals.pb(mi);
    }
    ...
    ff(x, 2, 2 * k)
    {
        ll gx = (lower_bound(all(maxvals), x - k) - begin(maxvals));
        ll lx = sz(minvals) - (lower_bound(all(minvals), x) - begin(minvals));
        // pr(gx, lx);
        res = min(res, 2 * (gx + lx) + (n / 2 - sums[x] - (gx + lx)));
    }
    print(res);
}