...
void solve()
{
    ...
    swap(v[1], v[k]); // reverting back
    int first_enemy_pos = 1;
    while (v[first_enemy_pos] < v[k])
        first_enemy_pos++;
    int case2 = 0;
    if (first_enemy_pos != 1)
        case2++; // 1 win from the back
    for (int i = first_enemy_pos + 1; i < k; i++)
    {
        if (v[i] < v[k])
            case2++;
        else
