...
void solve()
{
    ...
    vll critical_points;
    for (auto [a, b] : pre)
        critical_points.pb(a);
    ll intersections = 0;
    MInt<P> res = 0;
    ff(i, 0, sz(critical_points) - 2)
    {
        intersections += pre[critical_points[i]];
        ll len = critical_points[i + 1] - critical_points[i];
        if (intersections >= k)
            res += (len * c.binom(intersections, k).x);
    }
    print(res);
}
// ======================================================
