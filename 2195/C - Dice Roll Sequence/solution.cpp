...
//  Created      : 15.02.2026 at 20:50:03
...
void solve()
{
    ...
    vll v(n);
    cin >> v;
    vvll dp(n + 1, vll(10ll, -1ll));
    auto rec = [&](int idx, int prev, auto &&self) -> int
    {
        if (idx == n)
            return 0ll;
        if (prev != -1 and dp[idx][prev] != -1ll)
            return dp[idx][prev];
        int ans = INT_MAX;
        ff(x, 1, 6)
        {
            if (prev == -1 or (x != prev and x != (7ll - prev)))
            {
                int cost = (v[idx] == x ? 0ll : 1ll);
                ans = min(ans, cost + self(idx + 1, x, self));
            }
        }
 
        if (prev != -1)
