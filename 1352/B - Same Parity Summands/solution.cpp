...
bool canimakewitheven(int n, int k)
{
    int rem = n - 2 * k;
    if (rem < 0 or rem & 1)
        return false;
...
bool canimakewithodd(int n, int k)
{
    int rem = n - k;
    if (rem < 0 or rem & 1)
        return false;
...
void solve()
{
    int n, k;
    cin >> n >> k;
    if (canimakewitheven(n, k))
    {
        py;
        FoF(i, 1, k - 1) cout << 2 << sp;
        cout << 2 + (n - 2 * k) << endl;
    }
    else if (canimakewithodd(n, k))
    {
        py;
        FoF(i, 1, k - 1) cout << 1 << sp;
        cout << 1 + (n - k) << endl;
    }
    else
        pn;
